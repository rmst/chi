...
@experiment
def bdpg_chains(self: Experiment, logdir=None, env=3, heads=10, n=3, bootstrap=False):
  ...
  if boostrap:
    agent = BdpgAgent(env, ac, cr, heads=heads)
  else:
    agent = Ddpg(env, )
  ...
  for ep in range(100000):
    ...
    if ep % 20 == 0:
      print(f'Return of episode {ep} after timestep {agent.t}: {R} (head = {info["head"]}, threshold = {threshold})')
  #
